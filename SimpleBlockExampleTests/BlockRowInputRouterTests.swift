...
struct BlockRowInputRouterTests {
	...
	func returnKeyAtTailEmitsTailCaretInfo() {
		...
		guard case let .policy(.enter(info, isTail))? = router.action(for: event, textView: textView) else {
	...
	func spaceKeyRoutesCaretSnapshot() {
		...
		guard case let .insertSpace(info)? = router.action(for: event, textView: textView) else {
	...
	func leftArrowAtStartProducesArrowLeft() {
		...
		guard case let .policy(.arrowLeft(info))? = router.action(for: event, textView: textView) else {
	...
	func arrowUpOnFirstLineExposesTotalLines() {
		...
		guard case let .policy(.arrowUp(info))? = router.action(for: event, textView: textView) else {
