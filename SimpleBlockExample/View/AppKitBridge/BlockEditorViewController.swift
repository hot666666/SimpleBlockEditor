...
private extension BlockEditorViewController {
	func buildInitialRows() {
		manager.forEachInitialNode { index, node in
			insertRow(for: node, at: index)
		}
	}
	
	func observeNodeEvents() {
	...
	func handleNodeEvent(_ event: BlockNodeEvent) {
		switch event {
	...
	func makeRowController(for node: BlockNode) -> BlockRowController {
		BlockRowController(node: node, delegate: self)
	}
	
	func rowController(for node: BlockNode) -> BlockRowController {
	...
	func moveRow(id: UUID, from _: Int, to: Int) {
	...
	func insertRowView(_ view: NSView, at index: Int) {
	...
	func handleFocusChange(_ change: FocusChange) {
		switch change {
	...
	func applyFocusChange(_ change: FocusChange, source _: BlockRowController?) {
	...
	func command(for event: EditorEvent, node: BlockNode) -> EditCommand? {
...
extension BlockEditorViewController: BlockRowControllerDelegate {
	...
	func rowController(_ controller: BlockRowController, commandFor event: EditorEvent, node: BlockNode) -> EditCommand? {
