...
private extension BlockRowController {
	func configureTextView() {
		...
		textView.pointerDidFocusHandler = { [weak self] caret in
			guard let self else { return }
			self.delegate?.rowController(self, requestFocusChange: .otherNode(id: self.node.id, caret: caret))
		}
	...
	func handleTodoToggle(isChecked: Bool) {
