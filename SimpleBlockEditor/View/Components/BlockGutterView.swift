...
extension BlockGutterView {
  fileprivate func setUpViews() {
    ...
    widthConstraint = contentContainer.widthAnchor.constraint(
      equalToConstant: EditorStyle.Constants.gutterSize.width)

    NSLayoutConstraint.activate([
      contentTopConstraint,
      leadingConstraint,
      contentContainer.trailingAnchor.constraint(
        equalTo: trailingAnchor, constant: -EditorStyle.Constants.gutterSpacing),
  ...
  fileprivate func setUpBullet() {
    ...
    NSLayoutConstraint.activate([
      ...
      bulletView.centerYAnchor.constraint(
        equalTo: contentContainer.topAnchor, constant: EditorStyle.Constants.gutterSize.height / 2),
    ])
  }

  fileprivate func setUpOrdered() {
    ...
    NSLayoutConstraint.activate([
      ...
      orderedLabel.centerYAnchor.constraint(
        equalTo: contentContainer.topAnchor, constant: EditorStyle.Constants.gutterSize.height / 2),
    ])
  }

  fileprivate func setUpTodo() {
    todoButton.title = ""
    ...
    NSLayoutConstraint.activate([
      ...
      todoButton.centerYAnchor.constraint(
        equalTo: contentContainer.topAnchor, constant: EditorStyle.Constants.gutterSize.height / 2),
    ])
  }

  @objc fileprivate func handleToggle(_ sender: NSButton) {
    onToggle?(sender.state == .on)
  }

  fileprivate func show(view visible: NSView?) {
